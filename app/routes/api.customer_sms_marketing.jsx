...
const CUSTOMER_SMS_MARKETING_FRAGMENT = `
  id
  defaultPhoneNumber {
    phoneNumber
    marketingState
...
const GET_SMS_MARKETING_QUERY = `#graphql
  query CustomerSmsMarketing($id: ID!) {
    customer(id: $id) {
      ${CUSTOMER_SMS_MARKETING_FRAGMENT}
    }
  }
`;

const UPDATE_SMS_MARKETING_MUTATION = `#graphql
...
function formatSmsMarketing(customer) {
  const phoneNumber = customer?.defaultPhoneNumber;
  const marketingState = phoneNumber?.marketingState || "NOT_SUBSCRIBED";
  return {
    customerId: customer?.id || "",
    phone: phoneNumber?.phoneNumber || "",
    enabled: marketingState === "SUBSCRIBED",
    marketingState,
    marketingOptInLevel: phoneNumber?.marketingOptInLevel || "",
    consentUpdatedAt: phoneNumber?.marketingUpdatedAt || null,
  };
}
...
async function handleGet(request) {
  ...
  return jsonCors(
    request,
    { ok: true, ...formatSmsMarketing(customer) },
    200,
    METHODS,
  );
}
...
async function handleUpdate(request) {
  ...
  if (parsed.marketingState === "SUBSCRIBED") {
    smsMarketingConsent.marketingOptInLevel = "SINGLE_OPT_IN";
  }

  const { data } = await shopifyGraphql(
    auth.shop,
    auth.accessToken,
    UPDATE_SMS_MARKETING_MUTATION,
    {
      input: {
  ...
  const payload = data?.data?.customerSmsMarketingConsentUpdate;
  ...
  return jsonCors(
    request,
    { ok: true, ...formatSmsMarketing(payload?.customer) },
    200,
    METHODS,
  );
}
...
export async function loader({ request }) {
  ...
  try {
    ...
    return jsonCors(
      ...
      {
        ok: false,
        error: error.message || "Failed to load SMS marketing consent",
      },
      500,
      METHODS,
    );
...
export async function action({ request }) {
  ...
  try {
    ...
    return jsonCors(
      ...
      {
        ok: false,
        error: error.message || "Failed to update SMS marketing consent",
      },
      500,
      METHODS,
    );
